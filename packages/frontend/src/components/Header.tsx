...
const Header: React.FC = () => {
  const [notifications, setNotifications] =
    useState<INotificationWithEventTitle[]>();
      ...
      try {
        ...
        if (response.length > 0) {
          ...
          if ((events?.length as number) > 0) {
            ...
            setNotifications(
              updatedNotifications as INotificationWithEventTitle[]
            );
          }
        }
      } catch (error) {
  ...
  const updateNotificationstatus = async (
    data: INotificationWithEventTitle
  ) => {
    try {
      ...
      if (response) {
        const updatedNotifications = notifications?.filter(
          (notification) => notification.event_id !== data.event_id
        );
  ...
  return (
    <nav className="flex justify-between px-8 py-6 items-center shadow-lg bg-white h-min sticky top-0 w-full z-50">
      <div>
        <img
          src="https://cdn-icons-png.flaticon.com/128/16462/16462118.png"
          className="w-14 ms-5"
          alt=""
        />
      </div>
      <div className="flex justify-center text-xl font-medium items-center mt-1">
        <Link href="/" className="px-2 hover:text-gray-700 text-black">
          Home
        </Link>
        <Link href="/explore" className="px-2 hover:text-gray-700 text-black">
          Explore
        </Link>
        ...
        {(role === "organizer" || role === "admin") && (
          <Link
            href="/event/manage"
            className="px-2 hover:text-gray-700 text-black"
          >
            Manage Events
          </Link>
        )}
        ...
        {role === "admin" && (
          <Link href="/admin" className="px-2 hover:text-gray-700 text-black">
            Admin Panel
          </Link>
        )}
      </div>
